...
public class QueryBuilder {
   ...
   *
   * @param pSettingsID   L'arrangement de parametres selectionne dans la liste des parametres
   * @param pDatabaseName le nom de la BDD cible
   * @param pTargetTable  le nom de la table sur laquelle realiser le CRUD
   * @throws SQLException erreur SQL
   */
  public void createContext(Integer pSettingsID, String pDatabaseName, String pTargetTable) throws SQLException {
    final Connection conn = DriverManager.getConnection(url, userName, password);

    dslContext = DSL.using(conn, SQLDialect.MYSQL, getSettingsList().get(pSettingsID));

    final Meta metadata = dslContext.meta();
    final List<Table<?>> tables = metadata.getTables();
    for (Table<?> table : tables) {
      if (table.getName().equals(pTargetTable)) {
        targetTable = table;
      }
    }

    displaySchema(pDatabaseName, null);


  }

  /**
   * Pour afficher le schema de la base dans les logs
   *
   * @param pDatabaseName Nom de la BDD cible
   * @param pTableName    Nom de la Table cible
   */
  private void displaySchema(String pDatabaseName, String pTableName) {
    ...
    for (final Table<?> table : tables) {
      ...
      if (pTableName == null && table.getSchema().getName().equals(pDatabaseName)) {
        ...
        if (table.getName().equals(pTableName)) {
          LOGGER.info("TABLE ".concat(pTableName).concat(" CREATED"));
        }
   ...
   * @param pDatabaseName le nom de la BDD cible
   */
  public void jooqCreate(String pDatabaseName) {
    ...
    displaySchema(pDatabaseName, clientTable.getName());
  }
